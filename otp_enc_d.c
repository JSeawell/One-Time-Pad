...
one-time-pad (OTP) encription. This server receives
...
/*
This function takes an array of msg ints, and an
array of key ints, and creates an arr of encripted ints
*/
void encodeArrays(int* plainArr, int* keyArr, int size)
{
	int i = 0;
	for (i=0; i<size; i++){
		cyphArr[i] = (plainArr[i] + keyArr[i]) % 27;
	}
}
/*********************************************/
...
int main(int argc, char *argv[])
{
	...
	if (listenSocketFD < 0) { error("SERVER_ENC: ERROR opening socket"); exit(1); }

	// Enable the socket to begin listening
	if (bind(listenSocketFD, (struct sockaddr *)&serverAddress, sizeof(serverAddress)) < 0){ // Connect socket to port
		error("SERVER_ENC: ERROR on binding");
		exit(1);
	}
...
/*********************************************/      
		...
		if (establishedConnectionFD < 0) { 
			error("SERVER_ENC: ERROR on accept"); 
			//exit(1); 
		}			
...
/*********************************************/
        ...
        //child process runs this code
			...
			// Get size of plaintext from the client and put in file
			plainCount = -5;
			charsRead = recv(establishedConnectionFD, &plainCount, sizeof(int), 0);
			//printf("plain count received: %d\n", plainCount);		
			//If error
			if (charsRead < 0) error("SERVER_ENC: ERROR reading plaintext count from socket");
			...
			charsReadSum = 0;
			do {
				charsRead = recv(establishedConnectionFD, buffer, plainCount, 0); // Read the client's message from the socket
				if (charsRead < 0) 
					error("SERVER_ENC: ERROR reading plaintext from socket");
				else
			...
			bufferToInt(otherBuffer, plainArr);
			//Reset otherBuffer
			...
			//printf("key count received: %d\n", keyCount);
			//If error
			if (charsRead < 0) error("SERVER_ENC: ERROR reading key count from socket");
			...
			do {
				...
				if (charsRead < 0) 
					error("SERVER_ENC: ERROR reading key from socket");
				else
					charsReadSum = charsReadSum + charsRead;	
				strcat(otherBuffer, buffer);
				memset(buffer, '\0', BUFFER_SIZE);
			...
			encodeArrays(plainArr, keyArr, plainCount);

			//Print contents of cypher array as chars
			printIntToChar(buffer2, cyphArr, plainCount);
			
            /*********************************************/          
            
			// Send encoded message back to the client
			charsRead = send(establishedConnectionFD, buffer2, plainCount, 0); // Send success back
			if (charsRead < 0) error("SERVER_ENC: ERROR writing to socket");			
